...
from .groundingdino.transformer_vanilla import TransformerEncoderLayer
from .groundingdino.fuse_modules import BiAttentionBlock
from .groundingdino.utils import get_sine_pos_embed
...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.encoder = MSDeformAttnTransformerEncoder(encoder_layer, text_enhance_layer, feature_fusion_layer, num_encoder_layers)
    ...
    def forward(self, srcs, masks, text_dict, pos_embeds, use_ckpt=False):
        ...
        # encoder
        memory, memory_text = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, 
                                           lvl_pos_embed_flatten, mask_flatten, text_dict, use_ckpt=use_ckpt)
        return memory, spatial_shapes, level_start_index, mask_flatten, memory_text
...
class MSDeformAttnTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, text_enhance_layer, feature_fusion_layer, num_layers):
        super().__init__()
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, text_feats=None, use_ckpt=False):
        memory_text = text_feats
        bs, n_text, text_dim = memory_text.shape
        text_attention_mask = torch.zeros((bs, memory_text.shape[1])).bool().to(text_feats.device)
        pos_text = None
        text_self_attention_masks = torch.stack(
            [torch.eye(memory_text.shape[1]) for i in range(0, bs)]).bool().to(text_feats.device)
        position_ids = torch.stack(
            [torch.zeros((memory_text.shape[1])) for i in range(0, bs)]).to(text_feats.device)
        ...
        for layer_id, layer in enumerate(self.layers):
            if use_ckpt:
                ...
                output, memory_text = self.fusion_layers[layer_id](
                    v=output,
                    l=memory_text,
                    attention_mask_v=padding_mask, #key_padding_mask,
...
class MaskDINOEarlyFusion(nn.Module):
    @configurable
    def __init__(
        self,
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ] = enc_cfg['TRANSFORMER_ENC_LAYERS']
    ...
    # def forward_features(self, features, text_embeds, masks, use_ckpt=False):
    def forward_features(self, features, masks, text_embeds, use_ckpt=False):
        """
        ...
        y, spatial_shapes, level_start_index, mask_flatten, memory_text = self.transformer(srcs, masks, text_embeds, pos, use_ckpt=use_ckpt)
        bs = y.shape[0]
        ...
        for o in out:
            if num_cur_levels < self.total_num_feature_levels:
                ...
                num_cur_levels += 1
        return self.mask_features(out[-1]), out[0], multi_scale_features, memory_text