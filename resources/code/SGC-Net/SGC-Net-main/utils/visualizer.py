...
#         vis_images = images.permute(0, 2, 3, 1).detach().cpu().numpy()
...
#             pboxes[:, 0::2] = pboxes[:, 0::2] 
#             pboxes[:, 1::2] = pboxes[:, 1::2] 
#             oboxes[:, 0::2] = oboxes[:, 0::2] 
#             oboxes[:, 1::2] = oboxes[:, 1::2] 
...
#                 token_id = keep[0][i]
#                 attn = attn_map[token_id, 12:].view(1, h // self.patch_size, w // self.patch_size)
#                 attn = attn - attn.min()
#                 attn = attn / attn.max()
...
#                 for layer in range(len(outputs["visual_atten"])):
#                     attn_map = outputs["visual_atten"][layer]
#                     attn_map_normal = torch.nn.functional.normalize(attn_map,p=2, dim=1)
#                     plt.imshow(attn_map_normal.squeeze(0).detach().cpu().numpy(), cmap='hot', interpolation='nearest')
#                     # attn_ = F.interpolate(attn.unsqueeze(0), scale_factor=self.patch_size, mode="nearest")[0][0].detach().cpu().numpy()
#                     plt.savefig(self.vis_dir.joinpath(f'image_{img_id}_attn_layer_{layer}_209.jpg'),format='jpg')
                
#                     for token_id in range(196):
#                         attn = attn_map[b, token_id, 13:].view(1, 224 // self.patch_size, 224 // self.patch_size)
#                         attn = attn - attn.min()
#                         attn = attn / attn.max()
#                         attn = F.interpolate(attn.unsqueeze(0), scale_factor=self.patch_size, mode="nearest")[0][0].detach().cpu().numpy()
#                         plt.imsave(self.vis_dir.joinpath(f'image_{img_id}_attn_layer_{layer}_token_id{token_id}.jpg'), arr=attn, format='jpg')
    ...
    def visualize_attention(self, images, targets, outputs, vis_threshold=0.1):
        ...
        for b in range(bs):
            ...
            for i in indices:
                ...
                attn_map = outputs["attn_maps"][b]
                token_id = keep[0][i]
                # attn = attn_map[token_id, :].view(1, h // self.patch_size, w // self.patch_size)
                attn = attn_map[token_id, 1:].view(1, 16, 14)
                attn = attn - attn.min()
                attn = attn / attn.max()
                attn = F.interpolate(attn.unsqueeze(0), scale_factor=self.patch_size, mode="nearest")[0][0].detach().cpu().numpy()
                plt.imsave(self.vis_dir.joinpath(f'image_{img_id}_hoi_{hoi_id}_score_{scores[i]:.2f}_attn.jpg'), arr=attn, format='jpg')
                ...
                for layer in range(len(outputs["visual_atten"])):
                    ...
                    for token_id in range(196):
                        if token_id == 170:
                            # attn = attn_map[b, token_id, 1:].view(1, 224 // self.patch_size, 224 // self.patch_size)
                            
                            attn = attn_map[b, token_id, 2:].view(1, 16, 14)
                            
                            attn = attn - attn.min()
                            attn = attn / attn.max()
                            attn = F.interpolate(attn.unsqueeze(0), scale_factor=self.patch_size, mode="nearest")[0][0].detach().cpu().numpy()
                            plt.imsave(self.vis_dir.joinpath(f'image_{img_id}_hoi_{hoi_id}_attn_layer_{layer}_token_id{token_id}.jpg'), arr=attn, format='jpg')
                        else:
                            pass
                print("SUCESS !")
