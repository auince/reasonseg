...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        if get_embeddings:
            ...
            self.dgw_proj = nn.Parameter(scale * torch.randn(width, output_dim))
            ...
            self.dgw_proj_layers =  nn.Parameter(scale * torch.randn(len(out_indices), width, output_dim))
    ...
    def forward(self, x: torch.Tensor, multi_scale : bool = False, f_idxs : list = []):
        ...
        if self.total_d_layer == 0: #shallow
            for i, blk in enumerate(self.transformer.resblocks):
                ...
                if len(self.out_indices) > 1:
                    if i in self.out_indices:
                        xp = x.permute(1, 0, 2)[:, 1:, :].permute(0, 2, 1)
                        features.append(xp.contiguous())
