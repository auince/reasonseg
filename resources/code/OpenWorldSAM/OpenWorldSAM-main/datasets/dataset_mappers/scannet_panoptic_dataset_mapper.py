...
class ScanNetPanoDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        dataset_dict['file_name'] = '_'.join(file_name.split('/')[-3:])
        # print(f"Dataset file name: {dataset_dict['file_name']}")
        ...
        if len(masks) == 0:
            ...
            max_prompts = 18  # Default maximum number of prompts
            ...
            dataset_dict["prompt"] = [text.replace('-other', '').replace('-merged', '').replace('-stuff', '') for text
                                      in class_names]
