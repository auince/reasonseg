...
_OPENWORLDSAM_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def _resolve_local_asset_path(path: str) -> str:
    ...
    if os.path.exists(path):
        return path
    rooted_path = os.path.join(_OPENWORLDSAM_ROOT, path)
    return rooted_path
...
class OpenWorldSAM2(nn.Module):
    ...
    def print_trainable_parameters(self):
        ...
        trainable_params = 0
        logger.info(
            f"{'Parameter Name':<40}{'Trainable':<10}{'Shape':<20}{'Num Params':<15}"
        )
        logger.info("=" * 85)
    ...
    def forward(self, batched_inputs, return_intermediate=False):
        """
        Args:
        ...
        for x in batched_inputs:
            ...
            if self.reasonseg_enabled and self.composition_mode == "composed_prompt":
                prompts = x.get("composed_prompt")
            if (
                prompts is None
                and self.reasonseg_enabled
                and self.composition_mode == "composed_prompt"
                and "query_struct" in x
            ):
                query_texts = x.get("query_text", x["prompt"])
                prompts = [
                    compose_reasonseg_prompt(query_struct, fallback_text=query_text)
                    for query_struct, query_text in zip(x["query_struct"], query_texts)
                ]
            if prompts is None:
                prompts = x["prompt"]
        ...
        """
        feat = torch.split(
            feat, [offset[i + 1] - offset[i] for i in range(len(offset) - 1)]
        )
        ...
        feats = [
            ...
            for feat, feat_size in zip(
                image_embeddings[::-1], self._bb_feat_sizes[::-1]
            )
        ][::-1]
        ...
        for img_idx in range(batch_size):
            ...
            if self.use_cross_attention:
                ...
                enhanced_batch_feat_with_tokens = self.cross_attention_transformer(
                    reshaped_batch_feat.unsqueeze(
                        0
                    ),  # Add batch dimension [1, num_tokens, embedding_dim]
                    img_embed,
                )
                # Remove batch dimension
                enhanced_batch_feat_with_tokens = (
                    enhanced_batch_feat_with_tokens.squeeze(0)
                )

                # Reshape back to original shape if needed
                if batch_feat_with_tokens.dim() == 2:
                    enhanced_batch_feat_with_tokens = (
                        enhanced_batch_feat_with_tokens.unsqueeze(1)
                    )

                # Skip connection
                batch_feat_with_tokens = (
                    original_batch_feat_with_tokens + enhanced_batch_feat_with_tokens
                )
            ...
            if not self.sam_iou:
                objectness_logits = self.objectness_prediction_head(
                    sam_tokens_out.squeeze(1)
                )
                outputs = {
                    "pred_masks": pred_masks.unsqueeze(0),
                    "pred_logits": objectness_logits.unsqueeze(0),
                }
            else:
                outputs = {
                    "pred_masks": pred_masks.unsqueeze(0),
                    "pred_logits": iou_pred.unsqueeze(0),
                }
            ...
            if not self.training:
                ...
                class_labels = torch.tensor(
                    [unique_categories[i] for i in class_indices],
                    dtype=torch.int64,
                    device=self.device,
                )
                ...
                if self.two_stage_inference:
                    ...
                    if keep_indices.sum() > 0:
                        ...
                        refined_masks, refined_iou_pred, refined_tokens_out, _ = (
                            self.visual_model.sam_mask_decoder(
                ...
                pred_masks = self.postprocess_masks(
                    low_res_masks, orig_hw=original_size_list[img_idx]
                )
                ...
                if self.refer_on:
                    ...
                    refer_masks, refer_scores = self.refer_inference(
                        pred_masks, pred_logits, class_labels
                    )
                ...
                if self.instance_on:
                    ...
                    prompt_results = self.instance_inference(
                        pred_masks, pred_logits, class_labels
                    )
                ...
                if self.semantic_on:
                    ...
                    mask_cls = torch.zeros(
                        (pred_masks.shape[0], num_classes + 1), device=self.device
                    )  # +1 for background
                    ...
                    sem_seg = self.semantic_inference(
                        mask_cls, pred_masks, keep_sem_bgd=False
                    )
    ...
    def instance_inference(self, pred_masks, iou_scores, class_labels):
        ...
        if top_k:
            ...
            )  # Ensure top_k does not exceed the number of masks
        ...
        if pred_masks.shape[0] == 0:
            ...
            print(
                "No valid masks remain after filtering. Returning an empty Instances object."
            )
        ...
        pred_boxes = bit_masks.get_bounding_boxes().to(
            device=self.device
...
class CrossAttentionTransformer(nn.Module):
    ...
    def forward(
        ...
        if input_dim != self.embedding_dim and self.input_projection is None:
            print(
                f"Creating input projection layer from {input_dim} to {self.embedding_dim}"
            )
        ...
        if image_dim != self.embedding_dim and self.image_projection is None:
            print(
                f"Creating image projection layer from {image_dim} to {self.embedding_dim}"
            )
        ...
        if self.input_projection is not None:
            ...
            if not hasattr(self, "output_projection") or self.output_projection is None:
                print(
                    f"Creating output projection layer from {self.embedding_dim} to {input_dim}"
                )
