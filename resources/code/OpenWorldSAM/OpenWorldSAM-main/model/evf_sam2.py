...
class EvfSam2Model(PreTrainedModel):
    ...
    def initialize_evf_modules(self, config):
        ...
        elif self.config.mm_extractor_scale == "large": # config is "large"
    ...
    def forward(
        ...
        **kwargs,
        ...
        # project the multimodal embeddings to the same dimension as the visual embeddings
    ...
    def inference(
        ...
        print("multi mask output", multimask_output)
        with torch.no_grad():
            backbone_out = self.visual_model.forward_image(images)
            # dict_keys(['vision_features', 'vision_pos_enc', 'backbone_fpn'])
            _, image_embeddings, _, _ = self.visual_model._prepare_backbone_features(backbone_out)
        ...
        if multimask_output:
            return pred_masks # [batch_size, num_masks, H_ori, W_ori]
        else:
            return pred_masks[:, 0]
